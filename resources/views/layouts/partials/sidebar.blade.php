<!-- Left side column. contains the logo and sidebar -->
<aside class="main-sidebar">

    <!-- sidebar: style can be found in sidebar.less -->
    <section class="sidebar">

        <!-- Sidebar user panel (optional) -->
        @if (! Auth::guest())
        <div class="user-panel">
            <div class="pull-left image">
                <img src="{{asset('/img/user2-160x160.jpg')}}" class="img-circle" alt="User Image" />
            </div>
            <div class="pull-left info">
                <p>{{ Auth::user()->name }}</p>
                <!-- Status -->
                <a href="#"><i class="fa fa-circle text-success"></i> {{ trans('adminlte_lang::message.online') }}</a>
            </div>
        </div>
        @endif

        <!-- search form (Optional) -->
        <form id="input_search_main_admin" action="#" method="get" class="sidebar-form">
            <div class="input-group">
                <input type="text" name="q" class="form-control" placeholder="{{ trans('adminlte_lang::message.search') }}..."/>
                <span class="input-group-btn">
                    <button type='submit' name='search' id='search-btn' class="btn btn-flat"><i class="fa fa-search"></i></button>
                </span>
            </div>
        </form>
        <!-- /.search form -->

        <!-- Sidebar Menu -->
        <ul class="sidebar-menu">
            <li class="header">{{ trans('adminlte_lang::message.header') }}</li>
            <!-- Optionally, you can add icons to the links -->
            <!--<li class="active"><a href="{{ url('/') }}"><i class='fa fa-link'></i> <span>{{ trans('adminlte_lang::message.home') }}</span></a></li>-->
            <li>
                <a href="{{url('admin/dashboard')}}">
                    <i class='fa fa-tachometer'></i>
                    <span>{{ trans('adminlte_lang::message.dblinkDashboard') }}</span>
                </a>
            </li>
            <li class="treeview">
                <a href="#"><i class='fa fa-music'></i> 
                    <span>{{ trans('adminlte_lang::message.dblinkTracks') }}</span> 
                    <i class="fa fa-angle-left pull-right"></i>
                </a>
                <ul class="treeview-menu">
                    <li>
                        <a href="{{url('admin/tracks/create')}}">
                            <i class='fa fa-circle-o'></i>
                            <span>{{ trans('adminlte_lang::message.dblinkTrackNueva') }}</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{url('admin/tracks')}}">
                            <i class='fa fa-circle-o'></i>
                            <span>{{ trans('adminlte_lang::message.dblinkTrackRegistradas') }}</span>
                        </a>
                    </li>
                </ul>
            </li>
            <!-- albume -->
            <li class="treeview">
                <a href="#"><i class='fa fa-folder-open'></i> 
                <span>{{ trans('adminlte_lang::message.dblinkAlbumes') }}</span> 
                    <i class="fa fa-angle-left pull-right"></i>
                </a>
                <ul class="treeview-menu">
                    <li>
                        <a href="{{url('admin/albumes/create')}}">
                            <i class='fa fa-circle-o'></i>
                            <span>Nuevo</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{url('admin/albumes')}}">
                            <i class='fa fa-circle-o'></i>
                            <span>Registrados</span>
                        </a>
                    </li>
                </ul>
            </li>
            <!-- albume -->
            <!-- Authors -->
            <li class="treeview">
                <a href="#"><i class='fa fa-paint-brush'></i> 
                    <span>Autores</span> 
                    <i class="fa fa-angle-left pull-right"></i>
                </a>
                <ul class="treeview-menu">
                    <li>
                        <a href="{{url('admin/authors/create')}}">
                            <i class='fa fa-circle-o'></i>
                            <span>Nuevo</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{url('admin/authors')}}">
                            <i class='fa fa-circle-o'></i>
                            <span>Registrados</span>
                        </a>
                    </li>
                </ul>
            </li>
            <!-- Authors -->
            <li>
                <a href="{{url('admin/users')}}">
                    <i class='fa fa-users'></i>
                    <span>{{ trans('adminlte_lang::message.dblinkUsers') }}</span>
                </a>
            </li>
            <!-- Authors -->
            <li>
                <a href="{{url('admin/review')}}">
                    <i class='fa fa-eye'></i>
                    <span>Revista</span>
                </a>
            </li>
            <!--<li class="treeview">
                <a href="#"><i class='fa fa-link'></i> <span>{{ trans('adminlte_lang::message.multilevel') }}</span> <i class="fa fa-angle-left pull-right"></i></a>
                <ul class="treeview-menu">
                    <li><a href="#">{{ trans('adminlte_lang::message.linklevel2') }}</a></li>
                    <li><a href="#">{{ trans('adminlte_lang::message.linklevel2') }}</a></li>
                </ul>
            </li>-->
        </ul><!-- /.sidebar-menu -->
    </section>
    <!-- /.sidebar -->
</aside>
